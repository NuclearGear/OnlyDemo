...
public class Utils {
    ...
    public static String formatInteger(int num) {
        ...
        char[] val = String.valueOf(num).toCharArray(); //参数转为char字符数组
        int len = val.length; //获取参数长度
        StringBuilder sb = new StringBuilder();//创建新的字符串对象
        for (int i = 0; i < len; i++) {//遍历字符数组
            String m = String.valueOf(val[i]);//转字符串
            int n = Integer.valueOf(m);//转int
            boolean isZero = n == 0;//判断是否等于0
            String unit = units[(len - 1) - i];//获取当前数字单位
            if (isZero) {
                if(i!=len-1){//判断是否最后一位
                    if ('0' == val[i + 1]) {
                        ...
                        //只有当前val[i]的下一个值val[i+1]不为0才输出零
                        sb.append(numArray[n]);
                    }
                }
            } else {
                sb.append(numArray[n]);//添加数字
                sb.append(unit);//添加单位
            }
        }
